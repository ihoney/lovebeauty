...
<script type="text/javascript">
    ...
    function order_sure(node) {
        ...
        var url = getRootPath() + "privateOrder/privateOrderSure.do";
        $.ajax({
            url: url,
            type: "POST",
            ...
            success: function (data) {
                $(node).parent("td").prev("td").prev("td").prev("td").text("交易成功");
    ...
    function order_abort(node) {
        var order_id = $(node).attr("orderId");
        var url = getRootPath() + "privateOrder/abortPrivateOrder.do";
        $.ajax({
            url: url,
            type: "POST",
            data: {"orderId": order_id},
    ...
    function page_callback(data) {
        ...
        for (var i = 0, j = orders.length; i < j; i++) {
            ...
            if (orders[i].state == 1) {
                trTag += '<a href="javascript:void(0);" orderId="' + orders[i].id + '" onclick="order_sure(this);">确认已付款</a>' +
                        '<a href = "javascript:void(0);" orderId = "' + orders[i].id + '" onclick = "order_abort(this);" > 取消 < /a>';
            }
            trTag += "</td>";
            trTag += '</tr>';
            tabTag.append($(trTag));
...
<div class="tab_header">
    <span class="fontStyle_bold cur_pos">你当前的位置：</span>[业务中心]-[我的私人订制列表]
</div>
<div id="tb_body">
            ...
            <tr class="tr_body">
                ...
                <td>
                    ...
                    <c:if test="${order.state == 1}">
                        <a href="javascript:void(0);" orderId="${order.id}" onclick="order_sure(this);">确认已付款</a>
                        <a href="javascript:void(0);" orderId="${order.id}" onclick="order_abort(this);">取消</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
